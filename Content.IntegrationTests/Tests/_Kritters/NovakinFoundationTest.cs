...
public sealed class NovakinFoundationTest
{
    ...
    public async Task ReserveThermalContainmentCurveTest()
    {
        ...
        await server.WaitPost(() =>
        {
            ...
            // Thermal containment follows the critical-reserve threshold, not
            // the separately configurable threshold for reserve-depletion damage.
            physiology.CriticalReserveThreshold = 0.5f;
            physiology.DamageThreshold = 0.25f;
            physiology.CurrentReserve = 50f;
        });
    ...
    [Test]
    public async Task DischargeCoreVentsReserveAndRequiresTemperature()
    {
        await using var pair = await PoolManager.GetServerClient();
        var server = pair.Server;
        var entities = server.ResolveDependency<IEntityManager>();
        ...
        await server.WaitPost(() =>
        {
            ...
            novakin = entities.SpawnEntity("MobNovakin",
                new EntityCoordinates(gridEntity!.Value.Owner, new Vector2(0.5f, 0.5f)));
            physiology = entities.GetComponent<NovakinPhysiologyComponent>(novakin);
            temperature = entities.GetComponent<TemperatureComponent>(novakin);
            mixture = entities.System<AtmosphereSystem>().GetContainingMixture(novakin)!;
            physiology.CurrentReserve = 100f;
            temperature.CurrentTemperature = 649f;
            initialNitrogen = mixture.GetMoles(Gas.Nitrogen);

            var tooCool = new NovakinDischargeEvent();
            entities.EventBus.RaiseLocalEvent(novakin, tooCool);
            Assert.That(tooCool.Handled, Is.False);
        });

        await server.WaitAssertion(() =>
        {
            Assert.That(physiology.CurrentReserve, Is.EqualTo(100f));
            Assert.That(temperature.CurrentTemperature, Is.EqualTo(649f));
            Assert.That(mixture.GetMoles(Gas.Nitrogen), Is.EqualTo(initialNitrogen).Within(0.0001f));
        });

        await server.WaitPost(() =>
        {
            temperature.CurrentTemperature = 650f;
            entities.EventBus.RaiseLocalEvent(novakin, new NovakinDischargeEvent());
        });

        await server.WaitAssertion(() =>
        {
            Assert.That(physiology.CurrentReserve, Is.EqualTo(50f).Within(0.001f));
            Assert.That(temperature.CurrentTemperature, Is.EqualTo(350f).Within(0.001f));
            Assert.That(mixture.GetMoles(Gas.Nitrogen),
                Is.EqualTo(initialNitrogen + 0.5f).Within(0.0001f));
        });
