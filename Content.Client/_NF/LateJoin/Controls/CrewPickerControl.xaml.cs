...
public sealed partial class CrewPickerControl : PickerControl
{
    ...
    public CrewPickerControl()
    {
        ...
        {
            PlaceHolder = Loc.GetString("frontier-lobby-crew-search-placeholder")
        };
        SearchBar.OnTextChanged += (evt) => UpdateUi(_lobbyJobs);
        StationItemList.AddChild(SearchBar);
    }
    ...
    public override void UpdateUi(IReadOnlyDictionary<NetEntity, StationJobInformation> obj)
    {
        ...
        foreach (var item in StationItemList.Children.ToList())
        {
            if (item is not LineEdit)
            {
                StationItemList.RemoveChild(item);
            }
        }
        ...
        IReadOnlyDictionary<NetEntity, StationJobInformation> obj)
    {
        ...
        foreach (var (stationEntity, stationJobInformation) in stationList)
        {
            if (_hideJoblessShips
                ...
                || string.IsNullOrEmpty(stationJobInformation.VesselDisplayInformation?.VesselAdvertisement)))
            {
                continue;
            }

            // Hide ship if search bar has text and text does not match a crew name or vessel name
            if (!string.IsNullOrEmpty(SearchBar.Text)
                && stationJobInformation.VesselDisplayInformation != null
                && (!stationJobInformation.VesselDisplayInformation.CrewNames.Values.Any(x => x.Contains(SearchBar.Text))
                && (!_prototypeManager.TryIndex<VesselPrototype>(stationJobInformation.VesselDisplayInformation?.Vessel, out var proto)
                || !proto.Name.Contains(SearchBar.Text))))
            {
                continue;
            }
