...
public sealed partial class ShuttleRecordsWindow : FancyWindow
{
    ...
    public Action<ShuttleRecord>? OnRetrieveShuttle;
    ...
    private void UpdateSelectedShuttleRecord(ShuttleRecord shuttleRecord)
    {
        ...
        if (isInStorage)
        {
            shuttleStatus = _loc.GetString(messageId: "shuttle-records-shuttle-status-stored");
        }
        else if (shuttleExists)
        {
            shuttleStatus = _loc.GetString(messageId: "shuttle-records-shuttle-status-active");
        }
        else
        {
        ...
        control.CopyDeedButton.Disabled = !shuttleExists || isInStorage;
        ...
        // Store button - only enabled if shuttle exists and is active (not already stored)
        control.StoreShuttleButton.Disabled = !shuttleExists || isInStorage;
        control.StoreShuttleButton.Visible = bluespaceDockingEnabled;
        control.StoreShuttleButton.OnPressed += _ =>
        {
            if (ButtonResetOn is null)
            {
                ButtonResetOn = _gameTiming.CurTime.Add(TimeSpan.FromSeconds(3));
                control.StoreShuttleButton.AddStyleClass(StyleBase.ButtonCaution);
                control.StoreShuttleButtonLabel.Text = Loc.GetString("shuttle-records-transaction-confirm");
                return;
            }

            ResetConfirmButton();
            OnStoreShuttle?.Invoke(shuttleRecord);
        };

        // Retrieve button - only enabled if shuttle is in storage
        control.RetrieveShuttleButton.Disabled = !isInStorage;
        control.RetrieveShuttleButton.Visible = bluespaceDockingEnabled;
        control.RetrieveShuttleButton.OnPressed += _ =>
        {
            OnRetrieveShuttle?.Invoke(shuttleRecord);
        };
