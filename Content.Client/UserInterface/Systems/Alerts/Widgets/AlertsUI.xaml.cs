...
public sealed partial class AlertsUI : UIWidget
{
        ...
        IReadOnlyDictionary<AlertKey, AlertState> alertStates)
    {
        foreach (var (alertKey, alertState) in alertStates)
        {
            ...
            {
                Sawmill.Warning("Found alert key without an alert type; alert keys should never be stored without an alert type set: {0}", alertKey);
                continue;
            }
            ...
            {
                Sawmill.Error("Unrecognized alert type {0}", alertType);
                continue;
            }
